...
*/
    ...
    public String getShopProducts(Model model, @PathVariable long idShop){
        ...
        model.addAttribute("idShop",idShop); //Se necesita para usarla en la url que redirige al new product dentro de una tienda
        return "showProducts";
    }
    ...
    @GetMapping("shops/{id}/redirectNewProduct")
    public String redirectNewProduct(@PathVariable long id, @RequestParam String name, @RequestParam float prize, @RequestParam String description, @RequestParam String image) {
        if (name.length() == 0)
            return "redirect:/error";
        if (image.length() == 0)
            image = "../assets/img/sudadera.png";
        Product product = new Product(name, description, prize, null, image, id); //Antes, donde ponemos id poníamos (long)1 y creo que esta mal
        productService.createProduct(product, id);
        return "redirect:/shops/{id}/products";
    }
    ...
    @GetMapping("/products/{id}/modifyProduct")
